...
class ControllerExtensionMazaLayoutBuilderEditSection extends Controller {
        public function index(){
                $this->load->language('extension/maza/layout_builder');
                $this->load->language('extension/maza/layout_builder/edit_section');
                ...
                $setting = maza\layout\Section::getSettings();
                ...
                foreach($screen_sizes as $size){
                    foreach ($setting[$size]['section_background_image'] as $code => $layer_background) {
                        foreach($languages as $language){
                            if(isset($layer_background['image'][$language['language_id']]) && is_file(DIR_IMAGE . $layer_background['image'][$language['language_id']])){
                                $setting[$size]['section_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize($layer_background['image'][$language['language_id']], 80, 80);
                            } else {
                                $setting[$size]['section_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize('no_image.png', 80, 80);
                            }
                        }
                    }
                }
                ...
                $data['section_types'] = array(
                        array('code' => 'full_width', 'name' => $this->language->get('text_full_width')),
                        array('code' => 'fixed_width', 'name' => $this->language->get('text_fixed_width')),
                        array('code' => 'fixed_content', 'name' => $this->language->get('text_fixed_content')),
                );

                $data['sticky_values'] = array(
		...
		$this->response->setOutput($this->load->view('extension/maza/layout_builder/edit_section', $data));
        }
}
