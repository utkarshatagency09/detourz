...
class ControllerExtensionMazaLayoutBuilderEditWidget extends Controller {
        public function index($setting = array()){
                $this->load->language('extension/maza/layout_builder');
                $this->load->language('extension/maza/layout_builder/edit_widget');
...
//                    $setting = maza\array_merge_subsequence($setting, $this->request->post);
                ...
                foreach($screen_sizes as $size){
                    foreach ($setting[$size]['widget_background_image'] as $code => $layer_background) {
                        foreach($languages as $language){
                            if(isset($layer_background['image'][$language['language_id']]) && is_file(DIR_IMAGE . $layer_background['image'][$language['language_id']])){
                                $setting[$size]['widget_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize($layer_background['image'][$language['language_id']], 80, 80);
                            } else {
                                $setting[$size]['widget_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize('no_image.png', 80, 80);
                            }
                        }
                    }
                }
		...
		$this->response->setOutput($this->load->view('extension/maza/layout_builder/edit_widget', $data));
        }
}
