...
class ControllerExtensionMazaLayoutBuilderEditModule extends Controller {
        public function index(){
                $this->load->language('extension/maza/layout_builder');
                $this->load->language('extension/maza/layout_builder/edit_module');
                ...
                $setting = maza\layout\Module::getSettings();
...
//                    $setting = maza\array_merge_subsequence($setting, $this->request->post);
                ...
                foreach($screen_sizes as $size){
                    foreach ($setting[$size]['module_background_image'] as $code => $layer_background) {
                        foreach($languages as $language){
                            if(isset($layer_background['image'][$language['language_id']]) && is_file(DIR_IMAGE . $layer_background['image'][$language['language_id']])){
                                $setting[$size]['module_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize($layer_background['image'][$language['language_id']], 80, 80);
                            } else {
                                $setting[$size]['module_background_image'][$code]['thumb'][$language['language_id']] = $this->model_tool_image->resize('no_image.png', 80, 80);
                            }
                        }
                    }
                }
                ...
                $data['caches'] = array(
                        array('code' => 'soft', 'name' => $this->language->get('text_soft')),
                        array('code' => 'hard', 'name' => $this->language->get('text_hard')),
                        array('code' => '0', 'name' => $this->language->get('text_disabled')),
                );
		...
		$this->response->setOutput($this->load->view('extension/maza/layout_builder/edit_module', $data));
        }
}
